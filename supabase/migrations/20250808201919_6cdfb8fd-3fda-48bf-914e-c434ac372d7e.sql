-- Seed dummy banking data without profiles (auth FK prevents profile inserts)
DO $$
DECLARE
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM public.accounts) = 0 THEN
    -- Create user ids to link related rows (note: not linked to auth.users)
    u1 := gen_random_uuid();
    u2 := gen_random_uuid();
    u3 := gen_random_uuid();
    u4 := gen_random_uuid();
